...
public class Utility {
	...
	/**
	 * @Title: getIconStream 
	 * @Description: 图片转换为字节流
	 * @param icon
	 * @param 设定文件 
	 * @return String  
	 */
	public static byte[] getIconStream(Bitmap icon){
        if (icon != null) {
            final int quality = 100;
            ByteArrayOutputStream stream = new ByteArrayOutputStream();
            icon.compress(CompressFormat.PNG, quality, stream);
            try {
				stream.close();
			} catch (IOException e) {
				e.printStackTrace();
    ...
    public static String getStringFromInput(InputStream inputStream) {
    	...
    	try {
    		String string = new String(bos.toByteArray(),"gbk");
            bos.close();
            return string;
        } catch (IOException e) {
            e.printStackTrace();
